...
| 需要长期、多阶段规划 | **roadmap-plan → roadmap-milestone → roadmap-sync** | `/spec-driven-roadmap-plan` → `/spec-driven-roadmap-milestone` → `/spec-driven-roadmap-sync` |
...
`/spec-driven-roadmap-plan`、`/spec-driven-roadmap-milestone` 和
`/spec-driven-roadmap-sync` 来维护 milestone 化的 roadmap。
...
拆分规划，并把 `Candidate Ideas` 与 `Planned Changes` 分开。milestone
完成状态只由关联 planned changes 是否归档来推导。

更具体的示例、修改方式和产物效果，见 [ROADMAP_GUIDE.md](ROADMAP_GUIDE.md)。
...
需要长期规划时，可用 **roadmap-plan**、**roadmap-milestone** 和
**roadmap-sync** 维护 milestone 化 roadmap。中途可用 **modify** 调整任意 artifact，用 **spec-content** 只改规格内容归位，用 **sync-specs** 把现有代码行为补回规格，用 **cancel** 放弃变更。
...
| `/spec-driven-roadmap-plan` | 创建或重构 `.spec-driven/roadmap/`，按 milestone 组织阶段目标 |
| `/spec-driven-roadmap-milestone` | 细化单个 milestone 的目标、候选想法、planned changes、风险与状态 |
| `/spec-driven-roadmap-sync` | 根据 active/archive changes 同步 roadmap 里的 milestone 状态 |
