...
5. **Identify specialized review checklists** — before issuing findings,
   ...
   replaces the baseline review checks. Treat reviewed code as author-agnostic:
   do not trust or soften review because the code may have been generated by AI,
   previously produced by you, or written by a human.
...
6. **Review code quality** — for each changed file, run the baseline review
   ...
   - **Impartiality**: judge the code that exists, not who wrote it; do not
     suppress, soften, or skip findings for AI-authored or self-authored code
...
- Review code based on evidence in the code and tests, never on authorship or
  assumed intent
...
- Never give self-authored or AI-authored code lighter treatment than
  human-authored code
