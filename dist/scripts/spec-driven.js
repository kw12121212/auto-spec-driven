...
function validateRoadmapIndex(roadmapDir, errors) {
    ...
    for (const line of lines.slice(milestoneHeadingIndex + 1)) {
        ...
        if (!match) {
            errors.push(`roadmap/INDEX.md entries must match '- [<file>](milestones/<file>) - <title> - <declared-status>' where <declared-status> is one of: ${DECLARED_ROADMAP_STATUSES.join(", ")}`);
            continue;
        }
...
function readTopLevelBulletItems(lines) {
    if (!lines)
        return [];
    return lines
...
function validatePlannedChangeLines(lines) {
    if (!lines)
        return null;
    for (const line of lines) {
        const trimmed = line.trim();
