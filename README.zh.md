...
| 需要长期、多阶段规划 | **roadmap-plan → roadmap-milestone → roadmap-recommend → roadmap-propose → roadmap-sync** | `/roadmap-plan` → `/roadmap-milestone` → `/roadmap-recommend` → `/roadmap-propose` → `/roadmap-sync` |
...
`/roadmap-plan`、`/roadmap-milestone`、`/roadmap-recommend`、`/roadmap-propose` 和 `/roadmap-sync` 来维护 milestone 化的 roadmap。
...
拆分规划，并把 `Candidate Ideas` 与 `Planned Changes` 分开。`roadmap-recommend`
...
**roadmap-recommend**、**roadmap-propose** 和 **roadmap-sync** 维护 milestone 化 roadmap。中途可用 **modify** 调整任意 artifact，用 **spec-content** 只改规格内容归位，用 **sync-specs** 把现有代码行为补回规格，用 **cancel** 放弃变更。
...
| `/roadmap-recommend` | 基于 roadmap 推荐下一个 change，等待用户接受或修改后再进入脚手架阶段 |
