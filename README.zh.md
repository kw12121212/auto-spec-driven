...
轻量级规范驱动开发框架：16 个 agent skills + 少量 TypeScript 脚手架。
...
- `brainstorm`、`propose`、`apply`、`spec-content` 和 `sync-specs` 都必须先读 INDEX.md 和相关规格文件，才能生成内容或写代码
...
- 每个 milestone 文件都定义阶段目标、完成标准、候选想法、已规划的
  changes、依赖、风险和推导出的状态
...
| AI 读取已有规格 | 明确要求：`brainstorm`、`propose`、`apply`、`spec-content` 和 `sync-specs` 必须先读 INDEX.md，再读所有相关规格文件，才能生成任何内容 | 无明确要求 |
...
| 需要长期、多阶段规划 | **roadmap-plan → roadmap-milestone → roadmap-brainstorm → roadmap-propose → roadmap-sync** | `/roadmap-plan` → `/roadmap-milestone` → `/roadmap-brainstorm` → `/roadmap-propose` → `/roadmap-sync` |
...
中途可用 `/spec-driven-modify` 调整 artifacts，用 `/spec-driven-spec-content` 精准放置规格内容；若代码已经先于 spec 演进，可用 `/spec-driven-sync-specs` 做补齐。

若需要在多个变更之上维护长期路线图，可用
`/roadmap-plan`、`/roadmap-milestone`、`/roadmap-brainstorm`、`/roadmap-propose` 和 `/roadmap-sync` 来维护 milestone 化的 roadmap。
...
拆分规划，并把 `Candidate Ideas` 与 `Planned Changes` 分开。`roadmap-brainstorm`
负责先推荐下一个 change，`roadmap-propose` 再把 planned work 转成普通
change，milestone 完成状态只由关联 planned changes 是否归档来推导。
...
```
init → [roadmap-plan / roadmap-milestone / roadmap-brainstorm / roadmap-propose / roadmap-sync] → [brainstorm] → propose → apply → verify → archive
```
...
**roadmap-brainstorm**、**roadmap-propose** 和 **roadmap-sync** 维护 milestone 化 roadmap。中途可用 **modify** 调整任意 artifact，用 **spec-content** 只改规格内容归位，用 **sync-specs** 把现有代码行为补回规格，用 **cancel** 放弃变更。
...
| `/roadmap-milestone` | 细化单个 milestone 的目标、候选想法、planned changes、风险与状态 |
| `/roadmap-brainstorm` | 基于 roadmap 推荐下一个 change，等待用户接受或修改后再进入脚手架阶段 |
| `/roadmap-propose` | 把某个 milestone 的 `Planned Changes` 条目转成 `.spec-driven/changes/` 下的普通 change |
...
│   └── milestones/
│       └── <milestone>.md   # 目标、完成标准、候选想法、planned changes、状态
├── specs/
