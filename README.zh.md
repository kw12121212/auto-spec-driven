...
轻量级规范驱动开发框架：21 个 agent skills + 少量 TypeScript 脚手架。
...
| `questions.md` | 开放/已解决问题 | 把歧义集中管理；propose 可以带着开放问题交接，apply 会把它们作为结构化阻塞项抛出，未解决问题仍会阻塞 verify/archive |

### 第四层：21 个技能——AI 行为的精确约束
...
| spec 与代码关系不清晰 | spec frontmatter 将每个规格文件映射到实现文件和测试文件 |
...
| 歧义追踪 | `questions.md` 集中管理开放问题；propose 可以带着它们交接，apply 会用结构化阻塞说明要求人工确认，未解决问题仍会阻塞 verify/archive | 无内置支持 |
| 运行时依赖 | 只用 Node.js 标准库，一个 TypeScript CLI 文件 | 全局 npm 包（`npm install -g @fission-ai/openspec`，需 Node 20.19+） |
...
> **完整快速开始指南：** [QUICKSTART.zh.md](QUICKSTART.zh.md) — 安装、初始化、第一个变更、常用命令。
...
`-g` 表示全局安装，所有项目共享同一份 skills。`skills` 提示你选择支持的 AI 工具时，按自己实际使用的工具勾选即可。
...
| 快速修复，无需正式 change | **simple-task** | `/spec-driven-simple-task 修复 README 里的拼写错误` |
...
自动执行 propose → apply → verify → review → archive，包含一个强制的 proposal 确认点，以及由阻塞条件触发的额外确认（例如 open questions 或空 delta archive 决策）。在 verify 阶段，如果存在实现文件或直接验证行为的测试文件，会按条件运行 mapping / unmapped-evidence 审计；在 review 阶段，默认复用该结果，只有相关证据集合变化时才重跑。若范围模糊，会建议先 brainstorm 再 auto。

对于 open questions，流程会用 `Question`、`Explanation`、`Impact`、`Recommendation` 的结构化格式向用户说明阻塞点，再等待显式确认。

### 2. 轻量任务流程（快速修复）

适合不需要走正式 change 流程的临时工作——调试、文档调整、配置修改：

```bash
/spec-driven-simple-task 修复 utils.ts 里的日期格式 bug
```

它会加载规格上下文后直接执行任务，完成后评估是否影响了 specs。不会在 `.spec-driven/changes/` 下创建任何条目。如果任务太大、范围模糊或属于紧急修复，会引导至合适的流程。完成的任务会记录到 `.spec-driven/simple-tasks/YYYY-MM-DD-<name>.md`。
...
如果进入 `/spec-driven-apply` 时 `questions.md` 里还有 open items，技能会在真正写代码前停下，并用 `Question`、`Explanation`、`Impact`、`Recommendation` 四个字段逐条展示阻塞问题，等待用户明确决策。
...
4. **apply** — 逐任务实现；若存在 open questions，先以结构化 blocker 形式向用户请求确认，再继续更新 delta specs 使其与实际实现一致
...
8. **ship** — 在所有质量门禁完成后，可选地提交并推送已归档且 roadmap 已同步的变更
...
| `/spec-driven-apply` | 逐任务实现；如果存在 open questions，会先用结构化 blocker 请求用户确认，再继续更新 delta specs |
...
| `/spec-driven-ship` | 在正常工作流门禁完成后，可选地提交并推送已归档且 roadmap 已同步的变更。 |
| `/spec-driven-simple-task` | 执行轻量临时任务（调试、文档、配置微调），不走正式 change 流程。完成后评估规格影响并记录到 `.spec-driven/simple-tasks/`。 |
...
默认必须的确认点在 proposal 之后；如果遇到 open questions、空 delta archive 决策等阻塞条件，还需要额外的显式确认。Auto 不会提交或推送；归档后如需发布本次变更，使用 `/spec-driven-ship <name>`。
...
Mapping frontmatter 使用仓库相对路径。实现文件放在
`mapping.implementation`，测试文件放在 `mapping.tests`；不要把这些路径写进
requirement 正文。
...
node dist/scripts/spec-driven.js audit-unmapped-spec-evidence [--implementation <repo-path> ...] [--tests <repo-path> ...]  # 报告未被任何主 spec 映射的候选文件 → JSON
