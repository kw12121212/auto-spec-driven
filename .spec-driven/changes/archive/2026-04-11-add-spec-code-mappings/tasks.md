...
- [x] Add `skills/spec-driven-remap-specs/SKILL.md` for retrofitting old spec mappings and repairing stale or malformed mapping frontmatter after confirmation.
- [x] Update `install.sh` and related installation tests so `spec-driven-remap-specs` is installed with the other bundled skills.
...
- [x] Verify `spec-driven-remap-specs` does not edit implementation files or requirement bodies when only mapping repair is requested.
